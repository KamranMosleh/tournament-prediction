...
export async function generateLatestDailyPunditSummaryForLeague(
  ...
  localDate?: string,
  supabaseArg?: ServiceClient,
  options: ForceGenerationOptions = {}
): Promise<SummaryGenerationResult> {
  if (!process.env.GROQ_API_KEY) {
    return { status: 'skipped', reason: 'GROQ_API_KEY not set' }
  }
  ...
  if (existing?.coverage_fingerprint === fingerprint && !options.force) {
...
export async function generateMatchRecap(
  ...
  const alreadyCurrent = existingRecapMatchesPrompt(existingRecap, actualResult, promptPlayers)
  if (alreadyCurrent && !options.force) {
    return { status: 'skipped', reason: 'already current' }
  }

  if (!hasGroq) {
    if (existingRecap && !alreadyCurrent) {
