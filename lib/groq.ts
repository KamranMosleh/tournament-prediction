...
export function insightPrompt(
  ...
  return `You are a concise football analyst. Write exactly 3 short sentences about the upcoming match: ${homeTeam} vs ${awayTeam} in the ${tournament} (${stage.replace(/_/g, ' ')}).${contextBlock}\nUse the team names exactly as provided: "${homeTeam}" and "${awayTeam}". Never write TBD, TBA, or any other placeholder. Use clear intermediate English for international users. Keep each sentence easy to read. Simple and common football jargon is allowed, but avoid advanced or technical terms. Focus on current form, key stats, or what makes this match interesting. Be specific and factual. Do not mention predictions or scores.`
}
